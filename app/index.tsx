export default {
  async fetch(request: Request, env: { OPENAI_API_KEY: string }) {
    // Basic CORS for web/Expo web; mobile native doesn't need it but it's harmless.
    const cors = {
    ...
    // Simple GET endpoints for QR/health checks
    if (request.method === "GET") {
      ...
      // For any other GET path, return 404 JSON
...
- If a repair is risky or advanced, say so and suggest a pro.`;
    ...
    // Use Chat Completions (stable and supported), returning a simple string.
    // You can switch to the newer Responses API later if you prefer.
